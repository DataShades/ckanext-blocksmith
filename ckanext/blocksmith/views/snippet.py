...
class EditorView(MethodView):
    ...
    def post(self):
        ...
        data_dict["extras"] = {}
        data_dict["extras"]["arguments"] = bs_utils.prepare_snippet_arguments(
            data_dict)

        try:
            tk.get_action('blocksmith_create_snippet')(
                make_context(), data_dict)
        except (tk.ValidationError) as e:
            tk.h.flash_error(e.error_summary)
            return tk.render("blocksmith/snippet/create.html", extra_vars={'data': data_dict})
...
class ReadView(MethodView):
    def get(self, snippet_id: str):
        ...
        try:
            tk.check_access("blocksmith_edit_snippet",
                            make_context(), {"id": snippet_id})
        except tk.NotAuthorized:
            return tk.abort(404, "Page not found")

        return tk.render("blocksmith/snippet/read.html", extra_vars={"snippet": snippet})
    ...
    def post(self, snippet_id: str):
        ...
        try:
            return tk.render("blocksmith/snippet/snippets/snippet_preview.html", extra_vars=extra_vars)
...
class EditView(MethodView):
    def get(self, snippet_id: str):
        ...
        return tk.render(
            "blocksmith/snippet/edit.html", extra_vars={"snippet_id": snippet_id, "data": snippet.dictize({})}
        )
    ...
    def post(self, snippet_id):
        ...
        try:
            tk.get_action('blocksmith_update_snippet')(
                make_context(), data_dict)
        except (tk.ValidationError) as e:
            tk.h.flash_error(e.error_summary)
            return tk.render("blocksmith/snippet/edit.html", extra_vars={"snippet_id": snippet_id, 'data': data_dict})
...
class ListView(MethodView):
    def get(self):
        ...
        snippets = [snippet.dictize({})
                    for snippet in model.SnippetModel.get_all()]

        return tk.render("blocksmith/snippet/list.html", extra_vars={"snippets": snippets})
...
bs_snippet_blueprint.add_url_rule(
    "/create", view_func=EditorView.as_view("create"))
bs_snippet_blueprint.add_url_rule("/edit/<snippet_id>", view_func=EditView.as_view("edit"))
bs_snippet_blueprint.add_url_rule("/read/<snippet_id>", view_func=ReadView.as_view("read"))
